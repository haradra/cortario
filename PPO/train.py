...
def get_args():
    ...
    parser.add_argument('--foveation', dest='foveation', action='store_true')
    parser.add_argument('--fix_x', type=float, default=1.0)
    parser.add_argument('--fix_y', type=float, default=1.0)
    parser.set_defaults(retina=True, foveation=False)
...
def train(opt):
    ...
    # Create environments
    envs = MultipleEnvironments(opt.world, opt.stage, opt.action_type, opt.num_processes, opt.retina_name, opt.fix_x, opt.fix_y, opt.retina, opt.foveation, None, opt.save_video)
