...
class CorticalPreprocessor(Wrapper):
    def __init__(self, env, retina_name, fix_x, fix_y):
        super(CorticalPreprocessor, self).__init__(env)
        self.cortical_mapper = CorticalMapping()
        self.cortical_mapper.setup_retina(retina_name=retina_name, fix_x=fix_x, fix_y=fix_y)
        self.cortical_mapper.setup_cortex(retina_name=retina_name)
...
class FoveationPreprocessor(Wrapper):
    def __init__(self, env, retina_name, fix_x, fix_y):
        super(FoveationPreprocessor, self).__init__(env)
        self.cortical_mapper = CorticalMapping()
        self.cortical_mapper.setup_retina(retina_name=retina_name, fix_x=fix_x, fix_y=fix_y)
        self.cortical_mapper.setup_cortex(retina_name=retina_name)
    ...
    def step(self, action):
        state, reward, done, info = self.env.step(action)
        transformed_state = self.cortical_mapper.backproject_transform(state)
        return transformed_state, reward, done, info
...
def create_train_env(actions, retina_name, fix_x, fix_y, output_path=None, mp_wrapper=True, save_video=False, use_retina=True, foveation=False):
    ...
    if use_retina:
        if foveation:
            env = FoveationPreprocessor(env, retina_name=retina_name, fix_x=fix_x, fix_y=fix_y)
        else:
            env = CorticalPreprocessor(env, retina_name=retina_name, fix_x=fix_x, fix_y=fix_y)
...
class MultipleEnvironments:
    def __init__(self, world, stage, action_type, num_envs, retina_name, fix_x, fix_y, use_retina, foveation, output_path=None, save_video=False):
        ...
        self.envs = [create_train_env(actions, output_path=output_path, save_video=save_video, retina_name=retina_name, fix_x=fix_x, fix_y=fix_y, use_retina=use_retina, foveation=foveation) for _ in range(num_envs)]
